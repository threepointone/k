...
(function() {
    ...
    function k(config) {
        ...
        if(config instanceof k) return obj;
        ...
        // import all registered tasks onto this instance
        this.task(tasks);

        // populate `this.files`
        this.files = this.config.files = this.config.files || _.map(glob.glob(path.join(this.config.src, '**/*.*')), function(path) {
            return {
                src: path
            };
        });
    ...
    _.extend(k.prototype, {
        ...
        // clones self and starts a new deferred chain
        chain: function() {
            return new D({
                host: this.clone()
            });
