...
module.exports = {
    ...
    compress: function(options, done) {
        ...
        var files = _.map(this.files, function(f) {
            ...
            var content = path.extname(f.src) === '.js' ? _.compressjs(f.content) : (path.extname(f.src) === '.css' ? _.compresscss(f.content) : f.content);
            // set on 'f' to pass on
            f.content = content;
            return {
                src: f.src,
        ...
        this.chain().write(files).meta(files, 'compressed').then(done);
    ...
    hashify: function(done) {
        ...
        this.chain().write(files).meta(files, 'hashed').then(done);
