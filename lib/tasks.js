    ...
    var tasks = {
        ...
        read: function(done) {
            ...
            _.each(this.files, function(f) {
                var ext = path.extname(f.src);
                ext = (ext === '.js' || ext === '.css' || ext=== '.json');
                f.content = fs.readFileSync(f.src, ext? 'utf8' : null);
            });
            done();
        },
        ...
        // take a bunch of css files, and replace url() paths with cdn versions. 
        ...
        cmd: function(cmd, done) {
            ...
            var cmd = cp.exec(cmd, function(error, stdout, stderr) {
                if(error) {
